...
`testplan` needs a **feature source** — where the requirements (each with a stable id) and *what
shipped* come from. Every other action needs the **feature directory** — where the campaign
artifacts (`qa-plan.md`, `qa-runs.md`, `qa-report.md`) live. A source is one of three kinds; all
three resolve to the same two roles, so everything after `testplan` is identical no matter how the
plan was derived.
...
2. **`--brief <file.md>`** — a single markdown brief you wrote by hand (`templates/feature-brief.md`),
   carrying the **Requirements** (each with a stable id) and a **What shipped** summary. The
   universal, dependency-free path for any feature NOT built with spec-kit — a bot-loop feature, a
   hotfix, a design doc. The feature directory is the brief's own directory; artifacts sit beside it.
3. **`--from linear:SKU-12,SKU-13,…`** — a convenience adapter that *produces* a brief: fetch the
   named tracker issues and their linked PRs using the session's tracker access (the Linear MCP
   tools if present, else `gh` for the GitHub issues/PRs that carry the `Linear: SKU-n` backlink),
   distil issue descriptions → Requirements and merged PRs → What shipped, write the assembled brief
   to `.anantys/qa/<slug>/brief.md`, and **confirm it with the operator before proceeding**. From
   there it is a `--brief`. If the session has no tracker access, say so and ask the operator to
   write a brief and use `--brief` — never invent requirements.

Never guess between two candidate sources. Ask.

All artifacts are written **inside the feature directory**, beside its source:
...
- **`--brief` / `--from linear:`:** read the brief. Its **Requirements** section is the requirements
  role (the ids assertions cite); its **What shipped** section is the `tasks.md` role. There is no
  Polish phase to drop. If a requirement carries no stable id, assign one (`R1`, `R2`, …) and write
  it back into the brief — ids are referenced by runs, notes and reports forever.
